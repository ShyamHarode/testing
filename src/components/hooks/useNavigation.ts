...
interface NavigationItem {
  ...
  current: (item: NavigationItem) => boolean;
...
const useNavigation = (withAllItems: boolean = false): UseNavigationReturn => {
  const router = useRouter();
  const { currentWebsiteId: orgWebsiteId } = useOrgWebsites();
  const backupWebsiteId = router?.query?.websiteId as string | undefined;
  ...
  const isCurrentSection = (item: NavigationItem): boolean => {
    ...
    return item.subNavigation.some((subItem) => {
      ...
      const resolvedHref = subItem.href.replace("[websiteId]", websiteId || "");
  ...
  const navigation: NavigationItem[] = [
    ...
    },
    {
      ...
      shortName: "SEO",
      key: "SEO",
      description: "Manage your offsite SEO strategy.",
      icon: Link,
      current: isCurrentSection,
      subNavigation: [],
      productIsLive: false,
  ...
  return {
    ...
    currentNavItem: filteredNavigation.find((item) => item.current(item)),
  };
};
