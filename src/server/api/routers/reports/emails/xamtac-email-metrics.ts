...
export const xamtacEmailMetricsAgencyRouter = createTRPCRouter({
  ...
  //       };
  //       const response = await fetch(
  //         `${env.XAMTAC_EMAIL_METRICS_API_URL}/api/agency-email-metrics/${emailType === "campaign" ? "get-bulk-emails" : emailType === "automation" ? "get-automation-emails" : "get-single-emails"}/?agencyId=${ctx.session.user.agency?.id}/clientId=${input.clientId}&${startDate ? `startDate=${startDate}/` : ""}${endDate ? `&endDate${endDate}/` : ""}&take=${page * 20}&skip=${(page - 1) * 20}`,
