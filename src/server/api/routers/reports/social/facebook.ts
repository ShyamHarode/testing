...
export const facebookSocialMetricsRouterForAgency = createTRPCRouter({
  facebookSocialClientData: protectedProcedure
    .input(
      z.object({
        token: z.string(),
      }),
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        const response = await fetch(
          env.BACKEND_API +
            backendApiPaths.METRICS_META_SOCIAL_PAGES_CLIENT_API,
          requestOptions,
        );
